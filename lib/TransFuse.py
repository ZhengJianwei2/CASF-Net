...
from .res2net50_26w_4s import res2net50_26w_4s as resnet
...
class BiFusion_block(nn.Module):
    def __init__(self, ch_1, ch_2, r_2, ch_int, ch_out, drop_rate=0.):
        ...
        # print(ch_1, ch_2, r_2, ch_int, ch_out)

    def forward(self, g, x):
        # bilinear pooling
        # print("g.shape,x.shape",g.shape,x.shape)
        W_g = self.W_g(g)
        W_x = self.W_x(x)
        bp = self.W(W_g*W_x)
...
class MultiHeadCrossAttention(MultiHeadAttention):
    ...
    def forward(self, Y, S):
        ...
        x = torch.bmm(A, V)   #BNC  #.permute(0, 2, 1)#.reshape(Yb, Sc, Yh, Yw)
...
class TransFuse_S(nn.Module):
    def __init__(self,num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        self.Fusion = BiFusion_block(ch_1=256, ch_2=64, r_2=4, ch_int=256, ch_out=256, drop_rate=drop_rate/2)
        ...
        self.auge = auge()
        self.cross1 = MultiHeadCrossAttention(1024, 512,ch_out=512, drop_rate=drop_rate/2,qkv_bias=True)
        self.cross2 = MultiHeadCrossAttention(1024, 320,ch_out=320, drop_rate=drop_rate/2,qkv_bias=True)
        self.cross3 = MultiHeadCrossAttention(512, 128,ch_out=128, drop_rate=drop_rate/2,qkv_bias=True)
        self.cross4 = MultiHeadCrossAttention(256, 64,ch_out=64, drop_rate=drop_rate/2,qkv_bias=True)

        self.up1 = Up(512, 320, 320, attn=True)
        self.up2 = Up(320, 128, 128, attn=True)
        self.up3 = Up(128, 64, 64, attn=True)
        # low-level & high-level
        self.Translayer1_g = BasicConv2d(512, 320, 1)
        self.fam54_1 = decode(320, 320,320)
        ...
        self.Translayer1_l = BasicConv2d(512, 320, 1)
        self.fam54_2 = decode(320, 320,320)
    ...
    def forward(self, imgs, labels=None):
        ...
        t4 = pvt[3]
        pvt.append(t4)
        # print("tshape",pvt[0].shape,t2.shape,t3.shape,t4.shape)

        c0 = self.resnet.conv1(imgs)#torch.Size([1, 64, 176, 176])
        ...
        c3 = self.drop(c3)
        res.append(c3)
        c4 = self.resnet.maxpool(c3)#
        c4=self.resnet.layer4(c4)
        res.append(c4)

        cross_1,curg_1,curl_1=self.cross1(res[4],pvt[3])#[2, 512, 11, 11] 
        cross_2,curg_2,curl_2=self.cross2(res[3],pvt[2])#[2, 320, 22, 22]
        cross_3,curg_3,curl_3=self.cross3(res[2],pvt[1])#[2, 128, 44, 44]
        cross_4,curg_4,curl_4=self.cross4(res[1],pvt[0])#[2, 64, 88, 88]

        x_up_1 = self.up1(cross_1,cross_2)#[2, 320, 22, 22]
        x_up_2 = self.up2(x_up_1,cross_3) #[2, 128, 44, 44]
        x_up_3 = self.up3(x_up_2,cross_4) #[2, 64, 88, 88]                                 #decoder rdio the most

        out4_g = self.fam54_1(curg_2, self.Translayer1_g(curg_1))
        out3_g = self.fam43_1(curg_3, self.Translayer2_g(out4_g))
        out2_g = self.fam32_1(curg_4, self.Translayer3_g(out3_g))

        out4_l = self.fam54_2(curl_2, self.Translayer1_l(curl_1))
        out3_l = self.fam43_2(curl_3, self.Translayer2_l(out4_l))
        out2_l = self.fam32_2(curl_4, self.Translayer3_l(out3_l))


        map_x = F.interpolate(self.final_2(out2_g), scale_factor=4, mode='bilinear')
        map_1 = F.interpolate(self.final_1(out2_l), scale_factor=4, mode='bilinear')
        map_2 = F.interpolate(self.final_x(x_up_3), scale_factor=4, mode='bilinear')
        # print("map_x, map_1, map_2:",map_x.shape, map_1.shape, map_2.shape)
    ...
    def init_weights(self):
        ...
        self.Translayer1_g.apply(init_weights)
        self.fam54_1.apply(init_weights)
        ...
        self.Translayer1_l.apply(init_weights)
        self.fam54_2.apply(init_weights)
...
class decoder(nn.Module):
    ...
    def forward(self, C, T):
        ...
        cross_1=self.cross1(C[4],T[3])
        cross_2=self.cross2(C[3],T[2])
        cross_3=self.cross3(C[2],T[1])
        cross_4=self.cross4(C[1],T[0])

        x_up_1 = self.up1(cross_1,cross_2)   
        x_up_2 = self.up2(x_up_1,cross_3) 
        x_up_3 = self.up3(x_up_2,cross_4)      

        return x_up_3,cross_1
