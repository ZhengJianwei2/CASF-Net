...
def res2net50_26w_4s(pretrained=False, **kwargs):
    ...
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth = 26, scale = 4, **kwargs)
    if pretrained:
        model.load_state_dict('pretrained/res2net50_26w_4s-06e79181.pth')#(model_zoo.load_url(model_urls['res2net50_26w_4s']))
    return model
...
if __name__ == '__main__':
    images = torch.rand(2, 3, 352,352).cuda(0)
    model = res2net101_26w_4s(pretrained=True)
    model = model.cuda(0)
    # print(model(images).size())
    print(model)
    x_u=model.conv1(images)
    x_u = model.bn1(x_u)
    x_u = model.relu(x_u)
    x_u = model.maxpool(x_u)
    print(x_u.shape)
